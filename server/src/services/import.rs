...
/// Import subscription from in-memory content (browser file upload path).
/// Source is recorded as a `File`-style entry using a synthetic local name so
/// the store keeps working without a real path on the server.
pub fn import_from_content(
    name: Option<String>,
    content: &str,
) -> AppResult<ImportOutcome> {
    import_from_content_with_id(name, content, None)
}

/// Same as [`import_from_content`] but keeps a stable subscription id
/// (used when editing an existing upload-backed subscription).
pub fn import_from_content_with_id(
    name: Option<String>,
    content: &str,
    existing_id: Option<String>,
) -> AppResult<ImportOutcome> {
    ...
    let source = SubscriptionSource::File {
        path: format!("upload://{}", display_name),
    };
    let mut outcome = build_outcome(display_name, source, parsed, existing_id);
