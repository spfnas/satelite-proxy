...
pub fn update_settings(
    ...
    transparent_tcp_port: Option<u16>,
    transparent_udp_port: Option<u16>,
    ...
    let settings = state
        .with_store_mut(|store| {
            ...
            if let Some(t) = transparent_enabled {
                if store.settings.transparent_enabled != t {
                    transparent_changed = true;
                }
                store.settings.transparent_enabled = t;
                if t {
                    store.settings.capture_mode = crate::domain::CaptureMode::Transparent;
                } else if store.settings.capture_mode == crate::domain::CaptureMode::Transparent {
                    store.settings.capture_mode = crate::domain::CaptureMode::Off;
                }
            }
            if let Some(p) = transparent_tcp_port {
                if p > 0 && store.settings.transparent_tcp_port != p {
                    transparent_changed = true;
                    store.settings.transparent_tcp_port = p;
                }
            }
            if let Some(p) = transparent_udp_port {
                if p > 0 && store.settings.transparent_udp_port != p {
                    transparent_changed = true;
                    store.settings.transparent_udp_port = p;
                }
            }
    ...
    if let Some(enabled) = launch_changed {
        // Web backend: launch-at-login has no desktop session; store only.
        let _ = enabled;
    }
...
pub fn generate_singbox_config(state: State<'_, AppState>) -> Result<GenerateConfigResult, String> {
    ...
    let built = build_singbox_config(
        ...
        &BuildOptions {
            ...
            transparent_enabled: settings.transparent_enabled,
            transparent_tcp_port: settings.transparent_tcp_port,
            transparent_udp_port: settings.transparent_udp_port,
            dns,
