...
pub enum CaptureMode {
    ...
    System,
    Tun,
    /// Linux gateway transparent proxy: redirect (TCP) + tproxy (TCP/UDP)
    /// inbounds fed by nftables rules. Requires root + nft rules.
    Transparent,
}
...
impl CaptureMode {
    ...
    pub fn parse(value: &str) -> Option<Self> {
        match value.trim().to_ascii_lowercase().as_str() {
            ...
            "tun" => Some(Self::Tun),
            "transparent" | "tproxy" | "redirect" => Some(Self::Transparent),
            _ => None,
        }
    }
}
...
pub struct AppSettings {
    ...
    /// Enable transparent-proxy inbounds (redirect TCP + tproxy UDP) for Linux
    /// gateway setups. Requires root and nftables rules pointing LAN traffic here.
    #[serde(default)]
    pub transparent_enabled: bool,
    /// Port for the `redirect` inbound (TCP).
    #[serde(default = "default_transparent_tcp_port")]
    pub transparent_tcp_port: u16,
    /// Port for the `tproxy` inbound (TCP+UDP).
    #[serde(default = "default_transparent_udp_port")]
...
/// Default `redirect` inbound port (TCP) for transparent mode.
fn default_transparent_tcp_port() -> u16 {
    12345
}

/// Default `tproxy` inbound port (TCP+UDP) for transparent mode.
fn default_transparent_udp_port() -> u16 {
    12346
}
...
impl Default for AppSettings {
    fn default() -> Self {
        Self {
            ...
            transparent_tcp_port: default_transparent_tcp_port(),
            transparent_udp_port: default_transparent_udp_port(),
