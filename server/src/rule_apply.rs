...
//! restart workers.
//!
//! Web version: Tauri `AppHandle`/`Emitter` replaced with `Arc<AppState>` +
//! `EventBus` (see `crate::events`). Emitted events mirror the desktop names:
//! `rule-set-apply-status`, `config-apply-status`.
...
fn emit_config(bus: &EventBus, status: &'static str, error: Option<String>) {
    bus.emit_json(CONFIG_EVENT, &ConfigApplyStatusEvent { status, error });
}

fn emit(bus: &EventBus, id: &str, enabled: bool, status: &'static str, error: Option<String>) {
    bus.emit_json(
        EVENT,
        &ApplyStatusEvent {
            id: id.to_string(),
...
fn emit_batch(
    ...
    for (id, enabled) in batch {
        emit(bus, id, *enabled, status, error.clone());
    }
}
...
pub fn request_apply(state: Arc<AppState>, bus: EventBus, id: String, enabled: bool) {
    let start_worker = state.lock_rule_apply_queue().enqueue_toggle(id, enabled);
    if start_worker {
        spawn_worker(state, bus);
    }
}

/// Queue a non-toggle rule change for the same globally serialized restart.
pub fn request_restart(state: Arc<AppState>, bus: EventBus, cleanup_after_apply: Vec<PathBuf>) {
...
fn spawn_worker(state: Arc<AppState>, bus: EventBus) {
    tokio::task::spawn_blocking(move || loop {
        ...
        emit_batch(&bus, &batch.toggles, "restarting", None);
        if batch.generic_change {
            emit_config(&bus, "restarting", None);
        }
        ...
        match result {
            Ok(_) => {
                ...
                emit_batch(&bus, &terminal, "ready", None);
                if batch.generic_change {
                    emit_config(&bus, "ready", None);
                }
            }
