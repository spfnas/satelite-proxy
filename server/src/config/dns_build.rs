...
///
/// `transparent` disables FakeIP (LAN clients resolve real IPs via gateway DNS;
/// fakeip addrs would have no mapping in redirect/tproxy inbounds).
pub fn build_dns_section(
    ...
    if transparent_enabled {
        settings.fake_ip.enabled = false;
    }
...
mod tests {
    ...
    fn dns_final_follows_dns_final_setting() {
        ...
        let b = build_dns_section(&s, false, false, &[]);
    ...
    fn legacy_resolution_mode_no_longer_changes_dns_output() {
        ...
        let local_dns = build_dns_section(&local, false, false, &[]).dns;
        let old_smart_dns = build_dns_section(&old_smart, false, false, &[]).dns;
    ...
    fn enabled_legacy_dns_rules_are_preserved() {
        ...
        let b = build_dns_section(&s, false, false, &route);
