...
fn apply_running(app: &AppCtx) {
    crate::rule_apply::request_restart(app.state().clone(), app.bus().clone(), Vec::new());
}
...
pub fn set_rule_set_enabled(
    ...
    crate::rule_apply::request_apply(app.state().clone(), app.bus().clone(), id, enabled);
    Ok(())
}
...
pub async fn refresh_remote_rule_set(app: &AppCtx, id: String) -> Result<RuleSet, String> {
