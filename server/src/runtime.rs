...
pub struct ProxyStatus {
    ...
    /// Whether transparent-proxy inbounds are enabled in settings.
    #[serde(default)]
    pub transparent_enabled: bool,
    /// Persisted desired capture mode: off | system | tun | transparent.
...
impl Runtime {
    ...
    pub fn status(&mut self, store: &AppStore) -> ProxyStatus {
        ...
        ProxyStatus {
            ...
            transparent_enabled: store.settings.transparent_enabled,
    ...
    pub fn start_proxy(
        ...
        let built = build_singbox_config(
            ...
            &BuildOptions {
                ...
                transparent_enabled: store.settings.transparent_enabled,
                transparent_tcp_port: store.settings.transparent_tcp_port,
                transparent_udp_port: store.settings.transparent_udp_port,
                dns: store.dns.clone(),
        ...
        // Transparent inbounds bind raw sockets → root on Linux.
        let elevated = store.settings.tun_enabled || store.settings.transparent_enabled;
