...
fn st(app: &AppHandle) -> Result<State<'_, AppState>, String> {
...
async fn run_sync<T: serde::Serialize + Send + 'static>(
    app: AppHandle,
    args: Value,
    f: impl FnOnce(&AppHandle, &Value) -> Result<T, String> + Send + 'static,
) -> Result<Value, String> {
...
pub async fn dispatch(app: AppHandle, command: &str, args: Value) -> Result<Value, String> {
    match command {
        // ---------------- subscriptions ----------------
        "list_subscriptions" => {
            run_sync(app, args, |a, _| ok(commands::list_subscriptions(st(a)?))).await
        }
        "get_subscription" => run_sync(app, args, |a, args| {
        ...
        "add_subscription_file" => run_sync(app, args, |a, args| {
        ...
        "remove_subscription" => run_sync(app, args, |a, args| {
            ok(commands::remove_subscription(st(a)?, arg(args, "id")?))
        })
        .await,
        "list_subscription_nodes" => run_sync(app, args, |a, args| {
            ok(commands::list_subscription_nodes(st(a)?, arg(args, "id")?))
        })
        .await,
        "activate_subscription" => run_sync(app, args, |a, args| {
            ok(commands::activate_subscription(st(a)?, arg(args, "id")?))
        })
        .await,
        "set_mix_mode" => run_sync(app, args, |a, args| {
            ok(commands::set_mix_mode(st(a)?, arg(args, "mix")?))
        })
        .await,

        // ---------------- settings / config ----------------
        "get_settings" => run_sync(app, args, |a, _| ok(commands::get_settings(st(a)?))).await,
        "update_settings" => run_sync(app, args, |a, args| {
            let s = st(a)?;
        ...
        .await,
        "set_current_node" => run_sync(app, args, |a, args| {
            ok(commands::set_current_node(st(a)?, arg(args, "node_id")?))
        })
        .await,
        "list_all_nodes" => run_sync(app, args, |a, _| ok(commands::list_all_nodes(st(a)?))).await,
        "generate_singbox_config" => {
            run_sync(app, args, |a, _| ok(commands::generate_singbox_config(st(a)?))).await
        }
        "preview_singbox_config" => {
            run_sync(app, args, |a, _| ok(commands::preview_singbox_config(st(a)?))).await
        }
        "get_active_config_path" => {
            run_sync(app, args, |a, _| ok(commands::get_active_config_path(st(a)?))).await
        }

        // ---------------- core ----------------
        "get_core_info" => run_sync(app, args, |a, _| {
            let s = st(a)?;
            ok(commands::get_core_info(a.clone(), s))
        })
        .await,
        "check_core_update" => {
        ...
        "fetch_core_latest" => {
            ok(commands::fetch_core_latest().await.map_err(err_string)?)
        }

        // ---------------- proxy runtime ----------------
        "get_proxy_status" => run_sync(app, args, |a, _| {
            let s = st(a)?;
            ok(commands::get_proxy_status(a.clone(), s))
        })
        .await,
        "start_proxy" => run_sync(app, args, |a, args| {
            let s = st(a)?;
        ...
        .await,
        "stop_proxy" => run_sync(app, args, |a, _| {
            let s = st(a)?;
            ok(commands::stop_proxy(a.clone(), s))
        })
        .await,
        "restart_proxy" => run_sync(app, args, |a, _| {
            let s = st(a)?;
            ok(commands::restart_proxy(a.clone(), s))
        })
        .await,
        "set_system_proxy" => run_sync(app, args, |a, args| {
            let s = st(a)?;
            ok(commands::set_system_proxy(a.clone(), s, arg(args, "enabled")?))
        })
        .await,
        "set_tun_enabled" => run_sync(app, args, |a, args| {
            let s = st(a)?;
            ok(commands::set_tun_enabled(a.clone(), s, arg(args, "enabled")?))
        })
        .await,
        "set_capture_mode" => {
            let mode: String = arg(&args, "mode")?;
            ok(commands::set_capture_mode(app.clone(), mode)
                .await
                .map_err(err_string)?)
        }
        "set_outbound_mode" => run_sync(app, args, |a, args| {
            let s = st(a)?;
            ok(commands::set_outbound_mode(a.clone(), s, arg(args, "mode")?))
        })
        .await,
        "set_current_node_live" => run_sync(app, args, |a, args| {
        ...
        // ---------------- DNS ----------------
        "get_dns_settings" => run_sync(app, args, |a, _| ok(commands::get_dns_settings(st(a)?))).await,
        "update_dns_settings" => run_sync(app, args, |a, args| {
            let s = st(a)?;
            ...
            ok(commands::update_dns_settings(a.clone(), s, settings, apply))
        })
        .await,
        "reset_dns_defaults" => run_sync(app, args, |a, args| {
            let s = st(a)?;
        ...
        .await,
        "test_dns_lookup" => run_sync(app, args, |a, args| {
            ok(commands::test_dns_lookup(st(a)?, arg(args, "domain")?))
        })
        .await,
        "read_system_hosts" => run_sync(app, args, |_, _| ok(commands::read_system_hosts())).await,

        // ---------------- logs ----------------
        "list_app_logs" => run_sync(app, args, |_, args| {
        ...
        .await,
        "clear_app_logs" => run_sync(app, args, |_, _| ok(commands::clear_app_logs())).await,
        ...
        // ---------------- connections ----------------
        "list_connections" => run_sync(app, args, |a, _| ok(commands::list_connections(st(a)?))).await,
        "list_requests" => run_sync(app, args, |a, args| {
        ...
        .await,
        "list_request_failures" => run_sync(app, args, |a, args| {
        ...
        "clear_request_history" => {
            run_sync(app, args, |a, _| ok(commands::clear_request_history(st(a)?))).await
        }

        // ---------------- rule sets ----------------
        "list_rule_sets" => run_sync(app, args, |a, _| ok(commands::list_rule_sets(st(a)?))).await,
        "get_rule_set" => run_sync(app, args, |a, args| {
        ...
        "set_active_rule_set" => run_sync(app, args, |a, args| {
            let s = st(a)?;
            ok(commands::set_active_rule_set(a.clone(), s, arg(args, "id")?))
        })
        .await,
        "set_rule_set_enabled" => run_sync(app, args, |a, args| {
            let s = st(a)?;
        ...
        .await,
        "set_rule_set_strategy" => run_sync(app, args, |a, args| {
            let s = st(a)?;
        ...
        .await,
        "set_rule_set_dns_strategy" => run_sync(app, args, |a, args| {
            let s = st(a)?;
        ...
        .await,
        "reorder_rule_sets" => run_sync(app, args, |a, args| {
            let s = st(a)?;
            ok(commands::reorder_rule_sets(a.clone(), s, arg(args, "ids")?))
        })
        .await,
        "create_rule_set" => run_sync(app, args, |a, args| {
        ...
        .await,
        "update_rule_set" => run_sync(app, args, |a, args| {
        ...
        "refresh_remote_rule_set" => {
            ...
            ok(commands::refresh_remote_rule_set(app.clone(), id)
                .await
                .map_err(err_string)?)
        }
        "delete_rule_set" => run_sync(app, args, |a, args| {
            let s = st(a)?;
            ok(commands::delete_rule_set(a.clone(), s, arg(args, "id")?))
        })
        .await,
        "reset_rule_set" => run_sync(app, args, |a, args| {
            let s = st(a)?;
            ok(commands::reset_rule_set(a.clone(), s, arg(args, "id")?))
        })
        .await,
        "reset_builtin_rule_set" => run_sync(app, args, |a, _| {
            let s = st(a)?;
            ok(commands::reset_builtin_rule_set(a.clone(), s))
        })
        .await,

        // ---------------- rules ----------------
        "list_rules" => run_sync(app, args, |a, args| {
            ok(commands::list_rules(st(a)?, arg(args, "set_id")?))
        })
        .await,
        "save_rule" => run_sync(app, args, |a, args| {
            let s = st(a)?;
            let input: commands::SaveRuleInput =
                serde_json::from_value(arg_value(args, "input")?)
                    .map_err(|e| format!("bad input: {e}"))?;
            ok(commands::save_rule(a.clone(), s, input))
        })
        .await,
        "remove_rule" => run_sync(app, args, |a, args| {
            let s = st(a)?;
        ...
        .await,
        "set_rule_enabled" => run_sync(app, args, |a, args| {
            let s = st(a)?;
        ...
        // ---------------- lib-level commands ----------------
        "parse_subscription_text" => run_sync(app, args, |_, args| {
            ok(crate::parse_subscription_text(arg(args, "content")?))
        })
        .await,
        "set_ui_mode_pref" => run_sync(app, args, |a, args| {
            ok(crate::set_ui_mode_pref(a.clone(), arg(args, "mode")?))
        })
        .await,
        "peek_pending_import_urls" => {
            run_sync(app, args, |a, _| ok(crate::peek_pending_import_urls(st(a)?))).await
        }
        "clear_pending_import_urls" => run_sync(app, args, |a, _| {
            crate::clear_pending_import_urls(st(a)?);
            ok(())
        })
        .await,
