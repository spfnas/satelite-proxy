...
fn emit(bus: &EventBus, id: &str, status: &str, error: Option<String>) {
    bus.emit_json(
        EVENT,
        &StatusEvent {
            id: id.to_string(),
...
pub fn spawn(ctx: AppCtx) {
    tokio::spawn(async move {
        ...
        loop {
            ...
            for id in due {
                match refresh_download(&ctx, id.clone()).await {
                    Ok(downloaded) => {
                        changed = true;
...
fn due_ids(state: &AppState) -> Vec<String> {
    let now = now_secs();
    state
        .with_store(|store| {
            Ok(store
...
pub async fn refresh(app: &AppCtx, id: String) -> Result<RuleSet, String> {
    let downloaded = refresh_download(app, id).await?;
    crate::rule_apply::request_restart(
        app.state().clone(),
        app.bus().clone(),
        downloaded.cleanup_after_apply,
    );
    Ok(downloaded.set)
}
...
async fn refresh_download(app: &AppCtx, id: String) -> Result<DownloadedRule, String> {
    {
...
async fn refresh_inner(app: &AppCtx, id: &str) -> Result<DownloadedRule, String> {
    ...
    emit(app.bus(), id, "downloading", None);
    ...
    let cache_dir = app.app_data_dir.join("remote-rule-sets");
    ...
    let rule_count = match source_rule_count {
        Some(count) => count,
        None => {
            let resource_dir = app.resource_dir();
            let (core, _) =
                crate::core::resolve_core_bin(&state.app_data_dir, resource_dir);
...
fn fail<T>(app: &AppCtx, id: &str, error: String) -> Result<T, String> {
    let state = app.app_state();
    ...
    emit(app.bus(), id, "error", Some(error.clone()));
    Err(error)
}
