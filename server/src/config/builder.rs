...
pub struct BuildOptions {
    ...
    /// Enable transparent-proxy inbounds: redirect (TCP) + tproxy (TCP+UDP).
    pub transparent_enabled: bool,
    /// Port for the `redirect` inbound (TCP).
    pub transparent_tcp_port: u16,
    /// Port for the `tproxy` inbound (TCP+UDP).
    pub transparent_udp_port: u16,
...
pub fn build_singbox_config(nodes: &[ProxyNode], opts: &BuildOptions) -> AppResult<BuiltConfig> {
    ...
    // Transparent mode: LAN clients resolve via the gateway DNS (real IPs), so
    // FakeIP must be off — otherwise redirect/tproxy see fakeip addrs with no
    // mapping (missing fakeip record).
    let mut built_dns =
        build_dns_section(&opts.dns, opts.tun_enabled, opts.transparent_enabled, &effective_rules);
    ...
    if opts.transparent_enabled {
        // Linux gateway transparent proxy. LAN traffic must be steered here by
        // nftables/iptables (see server/deploy/nft-transparent.sh). `redirect`
        // handles TCP; `tproxy` handles TCP+UDP preserving the original dest.
        // Loopback is excluded so the app can still reach clash_api / mixed.
        // Note: sniff is handled by the global route rule `{action: sniff}` —
        // per-inbound `sniff` fields were removed in sing-box 1.13.
        inbounds.push(json!({
            "type": "redirect",
            "tag": "redirect-in",
            "listen": "0.0.0.0",
            "listen_port": opts.transparent_tcp_port
        }));
        inbounds.push(json!({
            "type": "tproxy",
            "tag": "tproxy-in",
            "listen": "0.0.0.0",
            "listen_port": opts.transparent_udp_port
        }));
    }
...
mod tests {
    ...
    #[test]
    fn builds_with_transparent_inbounds() {
        let nodes = vec![sample_ss()];
        let built = build_singbox_config(
            &nodes,
            &BuildOptions {
        ...
        .unwrap();
        let inbounds = built.value["inbounds"].as_array().unwrap();
        // mixed + redirect + tproxy
        assert_eq!(inbounds.len(), 3);
        let redirect = inbounds
            .iter()
            .find(|i| i["type"] == "redirect")
            .expect("redirect inbound");
        assert_eq!(redirect["listen_port"], 12345);
        let tproxy = inbounds
            .iter()
            .find(|i| i["type"] == "tproxy")
            .expect("tproxy inbound");
        assert_eq!(tproxy["listen_port"], 12346);
        // TUN must NOT be present when transparent only
